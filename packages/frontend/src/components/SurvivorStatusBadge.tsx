...
export default function SurvivorStatusBadge({ hasMintedNFT = false }: SurvivorStatusProps) {
  ...
  const scrollToProfile = () => {
    const el = document.getElementById('player-profile-badges') || document.getElementById('play-alpha');
  ...
  if (isConnected && mintedState) {
    return (
      <div 
        ...
        className="cursor-pointer group flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-[#D4AF37]/25 via-[#F3E5AB]/15 to-[#D4AF37]/25 border border-[#D4AF37] shadow-[0_0_15px_rgba(212,175,55,0.4)] hover:shadow-[0_0_25px_rgba(212,175,55,0.7)] transition-all duration-300"
        title="View Player Profile & Dynamic Badges"
      >
