...
export default function SurvivorStories() {
  ...
  }, [stories, selectedClassFilter, searchQuery]);
  ...
  const filteredStories = stories.filter((story) => {
    // 1. Filter by selected class archetype
    if (selectedClassFilter !== 'All' && story.survivorClass !== selectedClassFilter) {
      return false;
    }
    // 2. Filter by search input (author handle, author address, title, chronicle narrative, key lesson, or cycle)
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase().trim();
      const matchesAuthor =
        story.authorHandle.toLowerCase().includes(q) ||
        story.authorAddress.toLowerCase().includes(q);
      const matchesContent =
        story.title.toLowerCase().includes(q) ||
        story.chronicle.toLowerCase().includes(q) ||
        story.keyLesson.toLowerCase().includes(q) ||
        story.cycle.toLowerCase().includes(q) ||
        story.survivorClass.toLowerCase().includes(q);
      ...
      <div className="rounded-3xl bg-[#090D16] border-2 border-[#D4AF37]/40 p-6 sm:p-10 lg:p-12 gold-border-glow space-y-8">
        ...
        <div className="space-y-3">
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search chronicles by author handle, 0x address, or keywords (e.g. 'sudden death', 'gas', 'shield')..."
                className="w-full bg-[#080B12] text-xs font-mono text-white placeholder-[#F5F1E8]/40 pl-10 pr-9 py-2.5 rounded-xl border border-[#D4AF37]/35 focus:outline-none focus:border-[#D4AF37] focus:ring-1 focus:ring-[#D4AF37] transition-all shadow-inner"
              />
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => setSearchQuery('')}
                  className="absolute inset-y-0 right-0 pr-3 flex items-center text-[#F5F1E8]/50 hover:text-white cursor-pointer transition-colors"
                  title="Clear search"
                >
                  <X className="w-4 h-4" />
                </button>
            ...
            {searchQuery && (
              <div className="flex items-center gap-2 self-start md:self-auto">
                <span className="text-[11px] font-mono text-[#D4AF37] px-2.5 py-1 rounded-lg bg-[#D4AF37]/15 border border-[#D4AF37]/30 whitespace-nowrap">
                  Filtering: &ldquo;{searchQuery}&rdquo; ({filteredStories.length} found)
                </span>
                <button
                  onClick={() => setSearchQuery('')}
                  className="text-[11px] font-mono text-[#F5F1E8]/60 hover:text-white underline cursor-pointer"
                >
                  Clear
                </button>
              </div>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none text-[11px] font-mono">
            <span className="text-[#F5F1E8]/50 uppercase flex items-center gap-1 shrink-0 text-[10px]">
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg border transition-all cursor-pointer whitespace-nowrap ${
                  searchQuery.toLowerCase() === keyword.toLowerCase()
                    ...
                    : 'bg-[#06080F] text-[#F5F1E8]/70 border-[#D4AF37]/20 hover:border-[#D4AF37]/60 hover:text-white'
                }`}
              >
                #{keyword}
              </button>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
            <span className="text-xs font-mono text-[#F5F1E8]/50 uppercase mr-1 whitespace-nowrap">FILTER CLASS:</span>
        ...
        {/* Horizontal Scrolling Story Cards Track OR Empty State */}
        {filteredStories.length === 0 ? (
          <div className="py-14 px-6 rounded-2xl bg-[#080B12] border border-[#D4AF37]/30 text-center space-y-4">
            ...
            <div className="space-y-1">
              <h3 className="text-base font-bold text-white font-mono uppercase">
                NO SURVIVOR CHRONICLES FOUND
              </h3>
              <p className="text-xs text-[#F5F1E8]/70 font-sans max-w-md mx-auto">
                No chronicles matched your search for <span className="text-[#D4AF37] font-mono font-bold">&ldquo;{searchQuery}&rdquo;</span>
                {selectedClassFilter !== 'All' && <span> under class <strong className="text-white">{selectedClassFilter}</strong></span>}.
              </p>
            </div>
            <button
              onClick={() => {
              ...
              className="px-4 py-2 rounded-xl bg-[#D4AF37] hover:bg-[#F3E5AB] text-[#0A0A0A] font-mono text-xs font-bold uppercase transition-all cursor-pointer shadow-[0_0_15px_rgba(212,175,55,0.3)]"
            >
